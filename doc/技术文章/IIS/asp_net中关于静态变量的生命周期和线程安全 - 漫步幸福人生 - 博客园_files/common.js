...
function RefreshCommentList(parentId){$("#span_refresh_tips").html("正在刷新...");$("#span_refresh_tips").css("color","red");$("#lnk_RefreshComments").hide();var startId=$("#span_comment_maxid").html();if(!startId)startId=0;AjaxPost('/ws/CommentService.asmx/GetNewComments','{parentId:'+parentId+',startId:'+startId+'}',OnRefreshCommentList);return false;}
function OnRefreshCommentList(response){$("#span_refresh_tips").hide();$("#lnk_RefreshComments").show();$("#divCommentShow").html(response);}
...
function open_link(url){window.location=url;}
